...
namespace LinearBeats.EditorTests.Time
{
    ...
    {
        private readonly Sample _v0 = new Sample(default);
        ...
        private static void Iterate([NotNull] Action<float, Sample> action)
        {
            for (var i = 0; i < FloatTests.Iteration; ++i)
            {
                ...
                action(randomFloat, new Sample(randomFloat));
            }
        }
        ...
        [Test]
        public void Should_Implicit_Cast_To_Float()
        {
        ...
        [Test]
        public void Should_Implicit_Cast_From_Float()
        {
            Assert.AreEqual(_v0, (Sample) FloatTests.F0);
            Iterate((f, v) => Assert.AreEqual(v, (Sample) f));
        }

        [Test]
        [SuppressMessage("ReSharper", "SpecifyACultureInStringConversionExplicitly")]
        public void Should_Implicit_Cast_From_String()
        {
            Assert.AreEqual(_v0, (Sample) FloatTests.F0.ToString());

            Iterate((f, v) =>
            {
                var rand = (float) Math.Round(f, FloatTests.Digits);
                Assert.AreEqual(new Sample(rand), (Sample) rand.ToString());
            });
        }
        ...
        public void Implements_IComparable()
        {
            ...
            Iterate((f, v) =>
            {
                var positive = new Sample(f - 0.5f);
                var negative = new Sample(f + 0.5f);
