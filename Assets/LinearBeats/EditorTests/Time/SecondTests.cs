...
namespace LinearBeats.EditorTests.Time
{
    ...
    {
        private readonly Second _v0 = new Second(default);
        ...
        {
            Assert.AreEqual(_v0, (Second) FloatTests.F0);
        ...
        {
            Assert.AreEqual(_v0, (Second) FloatTests.F0.ToString());

            Iterate((f, v) =>
            {
                var rand = (float) Math.Round(f, FloatTests.Digits);
                Assert.AreEqual(new Second(rand), (Second) rand.ToString());
            });
        }
        ...
        public void Implements_IComparable()
        {
            ...
            Iterate((f, v) =>
            {
                var positive = new Second(f - 0.5f);
                var negative = new Second(f + 0.5f);
